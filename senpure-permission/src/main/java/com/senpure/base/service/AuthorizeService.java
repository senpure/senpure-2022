                    ...
                    if (beforePermission.getDatabaseUpdate()) {
                        ...
                        if (!permission.getServerName().equals(beforePermission.getServerName())) {
                            ...
                            if (beforePermission.getServerName().equals(PermissionConstant.DEFAULT_SERVER_NAME)) {
                                logger.info("权限{}服务更名 {} --> {}", beforePermission.getName(), beforePermission.getServerName(), permission.getServerName());
                            } else {
                                //使用其他服务器定义的权限
                                logger.info("权限{}  {} 使用 {} 的权限", beforePermission.getName(), permission.getServerName(), beforePermission.getServerName());
                                update = false;
                            }
                        }
                        if (update) {
                ...
                if (beforeMenu.getId().intValue() == menu.getId().intValue()) {
                    if (!beforeMenu.getServerName().equals(menu.getServerName())) {
                        if (beforeMenu.getServerName().equals(PermissionConstant.DEFAULT_SERVER_NAME)) {
                            logger.info("菜单{}服务更名 {} --> {}", beforeMenu.getText(), beforeMenu.getServerName(), menu.getServerName());

                        } else {
                            Assert.error("相同的菜单id存在于不同的服务 " + beforeMenu.getId() + " now " + menu.getServerName() + " before " + beforeMenu.getServerName());
                        }
                    }
                    save = false;
