...
@Component
@Order(value = 1)
public class PermissionsGenerator implements ApplicationRunner, ApplicationContextAware {
    ...
    @Value("${spring.application.name:"+PermissionConstant.DEFAULT_SERVER_NAME+"}")
    private String serverName;
    private final static Map<Integer, Menu> menuMap = new HashMap<>();
    @Resource
    ...
    public void run(ApplicationArguments args) {
        ...
        while (iterator.hasNext()) {
            ...
            if (permissionVerify != null) {
                ...
                permission.setServerName(serverName);
                ...
                if (resourceVerifies != null && resourceVerifies.length > 0) {
                    ...
                    if (permission.getName().endsWith("_owner")) {
                        ...
                        resourcePermission.setServerName(serverName);
            ...
            if (menuGenerator != null) {
                ...
                if (menuGenerator.parentId() == 0) {
                    ...
                    if (parent != null) {
                        ...
                        menuMap.putIfAbsent(parentMenu.getId(), parentMenu);
                    } else {
                        menu.setParentId(0);
                    }
                } else {
        ...
        extPermission(permissionMap, uriPermissionMap);
        createPermissions.addAll(permissionMap.values());
        if (!createPermissions.isEmpty()) {
            authorizeService.syncPermission(serverName, permissionMap.values(), uriPermissionMap);
        }
        if (menuMap.size() > 0) {
            for (Menu value : menuMap.values()) {
    ...
    private void extPermission(Map<String, Permission> permissionMap,
                               Map<String, List<URIPermission>> uriPermissionMap) {

        String[] beans = applicationContext.getBeanNamesForAnnotation(ExtPermission.class);
    ...
    public PermissionsGenerator setServerName(String serverName) {
