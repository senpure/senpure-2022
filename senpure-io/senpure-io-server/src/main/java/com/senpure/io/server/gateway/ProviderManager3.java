...
public class ProviderManager3 {
    ...
    public ProviderManager3(GatewayMessageExecutor messageExecutor) {
    ...
    public Provider getProducer(String producerKey) {
        for (Provider provider : useProviders) {
            if (provider.getRemoteServerKey().equals(producerKey)) {
                return provider;
            }
        }
    ...
    public synchronized void checkChannelServer(String serverKey, Provider channelManager) {
        for (Provider manager : useProviders) {
            if (manager.getRemoteServerKey().equals(serverKey)) {
                return;
            }
        }
