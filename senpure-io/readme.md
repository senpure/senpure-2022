...
| enum |  引用枚举   |   VARINT     |        去枚举的值进行var32               |  |
| bean |  引用bean   |   LENGTH_DELIMITED   | 先把bean中字段依次的进行编码获得bytes，再按bytes的方式进行编码                           |  |

### 字段属性说明

+ explain  string 字段注释
+  hasExplain 字段是否有注释 当explain不为空并且长度大于0时为true
+  name 字段名
+ baseField 是否属于基本数据类型 
+ list 是否是list列表
+ wireType   wire Type
+ 当字段为list 并且 list packed为true时会将wire Type按 LENGTH_DELIMITED 进行处理

|     字段     | base |list packed                                                           | 备注             |
|:------------|:----------|---------------------------------------------------------|:-----------------|
| int  |true  |true|      |
| long  |true  |true|      |
| sint  |true  |true|      |
