...
public class ContainerController extends BaseController {
    ...
    public ModelAndView readContainers(HttpServletRequest request,
       ...
       ContainerPageResult pageResult = containerService.findPage(criteria);
        ResultMap resultMap = ResultMap.result(pageResult.getCode());
        resultMap.putTotal(pageResult.getTotal());
        resultMap.putItems(ContainerPageResult.RECORDS_NAME, pageResult.getContainers());
    ...
    public ModelAndView createContainer(HttpServletRequest request, @ModelAttribute("criteria") ContainerCriteria criteria, BindingResult validResult) throws Exception {
        ...
        if (success) {
           ...
           ContainerPageResult pageResult = containerService.findPage(containerCriteria);
            ResultMap resultMap = ResultMap.result(pageResult.getCode());
            resultMap.putTotal(pageResult.getTotal());
            resultMap.putItems(ContainerPageResult.RECORDS_NAME, pageResult.getContainers());
